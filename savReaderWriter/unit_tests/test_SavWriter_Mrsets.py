...
class Test_Mrsets(unittest.TestCase):
    ...
    def test_multRespDefs_unicode_both(self):
        ...
        varNamesC = list(actual['testSetC']['varNames'])
        varNamesD = list(actual['testSetD']['varNames'])
        actual['testSetC']['varNames'] = varNamesC
        actual['testSetD']['varNames'] = varNamesD
        desired = \
        {'testSetC': {'label': '\u5927\u5b66 (labelC)',
                       'setType': 'C',
                       'varNames': ['income', 'educ']},
         'testSetD': {'countedValue': 'Yes',
                       'label': 'labelD',
                       'setType': 'D',
                       'varNames': ['income', 'educ']}}
        self.assertEqual(desired, actual)
