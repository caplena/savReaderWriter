...
class SavReaderNp(SavReader):
    ...
    @memoized_property
    def uvarTypes(self):
        """Returns a dictionary of variable names, as strings (keys)
    ...
    def datetimevars(self):
        """Returns a list of the datetime variable nanes (as strings)
    ...
    def _titles(self):
        ...
        titles =  [self.varLabels[v] if self.varLabels[v] else
                   ("col_%03d" % col).encode(self.encoding) for col, v in
                   enumerate(self.varNames)]
        return [title.decode(self.encoding) if not
                isinstance(title, str) else title for title in titles]
